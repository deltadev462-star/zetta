...
const LiveChat: React.FC = () => {
      ...
      if (!isSupport) {
        ...
        if (existingSession) {
          ...
          const { data: newSession, error } = await supabase
            ...
            .insert({
              customer_id: user?.id,
              customer_name: user?.profile?.full_name || user?.email || 'Customer',
              status: 'waiting',
            })
            .select()
            .single();
          ...
          if (!error && newSession) {
            ...
            const welcomeMessage: Message = {
              ...
              sender_name: 'System',
              sender_type: 'support',
              content: 'Welcome to Zetta Med support! A support agent will be with you shortly.',
  ...
  const sendMessage = async () => {
    ...
    const newMessage: Omit<Message, 'id'> = {
      sender_id: user?.id || '',
      sender_name: user?.profile?.full_name || user?.email || 'User',
    ...
    <Fade in={isOpen}>
      <Paper
        sx={{
          ...
          boxShadow: '0 16px 48px rgba(0,0,0,0.7), 0 0 80px rgba(0,212,255,0.2)',
          borderRadius: 4,
          overflow: 'hidden',
          zIndex: 10000,
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.9)' }}>
                  {chatSession?.status === 'active' ? 'Online' : 'We\'ll respond shortly'}
                </Typography>
              </Box>
            </Box>
          </Box>
                    ...
                    >
                      <Avatar
                        sx={{
                          ...
                          border: '2px solid',
                          borderColor: isCustomer ? 'rgba(0,102,204,0.3)' : 'rgba(0,212,255,0.3)',
                          boxShadow: isCustomer
                            ? '0 0 20px rgba(0,102,204,0.3)'
                            : '0 0 20px rgba(0,212,255,0.3)',
                        }}
                      >
                        {isCustomer ? <Person sx={{ fontSize: 20 }} /> : <Support sx={{ fontSize: 20 }} />}
                      </Avatar>
                      <Box>
                        <Paper
                          sx={{
